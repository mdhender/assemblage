...
## That is what makes it trustworthy enough for asmd to refuse to start over
...
## cmd/asmd/storage_test.go and internal/store/store_test.go manufacture a
...
no-version-writes:
	...
	if [ "$$writers" != "./cmd/asmd/storage_test.go ./internal/store/store_test.go " ]; then \
...
no-dev-routes:
	...
	$(GO) test -run 'TestDevRoutes|TestRouteTable' ./internal/server/
	@# Not filtered: every test in devroutes is about the gated routes, and a
	@# -run pattern that matches nothing reports "ok [no tests to run]" and
...
## dev: run asmd in development, behind the Caddy service, against ./var.
##
...
init:
	$(GO) run ./cmd/asmdb init --db $(DEV_DB)
...
## assigns it.
seed:
	$(GO) run ./cmd/asmdb seed --db $(DEV_DB)
...
NAME  ?= Admin
bootstrap:
	$(GO) run ./cmd/asmdb bootstrap admin --db $(DEV_DB) --email $(EMAIL) --name "$(NAME)"

## status: show the schema version and the applied and pending migrations.
status:
	$(GO) run ./cmd/asmdb migrate status --db $(DEV_DB)
...
## The tag adds one thing: buildenv.Verify panics unless ASSEMBLAGE_ENV=production
## is exported. It gates no routes and no features.
release:
