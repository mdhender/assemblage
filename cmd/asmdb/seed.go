...
import (
	...
	"github.com/mdhender/assemblage/internal/domain"
	"github.com/mdhender/assemblage/internal/events"
	"github.com/mdhender/assemblage/internal/ids"
	"github.com/mdhender/assemblage/internal/jobs"
	"github.com/mdhender/assemblage/internal/store"
	"github.com/spf13/cobra"
)

// The role slugs seed creates. They are constants because "asmdb bootstrap
...
// DefaultSiteDomain is the site "asmdb seed" creates when --site-domain does
// not say otherwise.
//
...
//
// A deployment says which it wants: "asmdb seed --site-domain www.example.com"
...
// is not.
//
// No field is required, deliberately. "asmdb seed" produces a system somebody
...
// (internal/migrate/schema/0001_initial.sql). DESIGN.md 5.4 says the default
// story workflow is seeded by "asmdb", and "asmdb init" is asmdb. What seed
...
func newSeedCmd() *cobra.Command {
	...
	cmd.Flags().BoolVar(&demo, "demo", false,
		"also seed one sample document and one queued job; needs \"asmdb bootstrap admin\" to have run")
...
func seed(ctx context.Context, db *store.DB, out io.Writer, siteDomain string) error {
	...
	// publication. Without it, "asmdb seed --site-domain www.example.com"
...
// user. That makes "asmdb bootstrap admin" a prerequisite, and the refusal
...
func seedDemo(ctx context.Context, db *store.DB, out io.Writer) error {
	...
	if err != nil {
		if errors.Is(err, domain.ErrNotFound) {
			return fmt.Errorf("--demo has nobody to attribute the sample content to; run \"asmdb bootstrap admin\" first")
		}
		return err
	}
...
// working, seeds one, and "asmd serve --workers 1" then runs it while you
...
// reason everything in asmdb is: this command owns no service, and the uid and
...
func seedDemoJob(ctx context.Context, db *store.DB, out io.Writer, authorID int64, now time.Time) error {
	...
	fmt.Fprintf(out, "demo: one %s job queued (%s); run \"asmd serve --workers 1\" to watch it run\n",
