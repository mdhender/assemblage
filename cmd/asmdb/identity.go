...
import (
	...
	"github.com/mdhender/assemblage/internal/authz"
...
// "asmdb bootstrap admin" and "asmdb seed" (DESIGN.md 11).
//
...
// has created one.
//
// A missing role is not an error: "asmdb init" then "asmdb bootstrap admin" is
...
func grantAdminRole(ctx context.Context, stderr io.Writer, db *store.DB, u domain.User) error {
	...
	if err != nil {
		if errors.Is(err, domain.ErrNotFound) {
			fmt.Fprintf(stderr,
				"note: there is no %q role yet, so no role was assigned; run \"asmdb seed\" and try again\n",
				AdminRoleSlug)
			return nil
		}
		return err
...
// | asmdb ..." produces, and nothing else: a password with a trailing space is
