...
func TestBackupWhileTheServerIsRunning(t *testing.T) {
	...
	stdout, stderr, code := run(t, bin["asmdb"], nil, "backup", "--db", dir, "--to", to)
	if code != 0 {
		t.Fatalf("asmdb backup exited %d while asmd was running\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	...
	for _, want := range []string{
		...
		if !strings.Contains(stdout, want) {
			t.Errorf("asmdb backup printed no %q\nstdout: %s", want, stdout)
		}
	}
	info, err := os.Stat(to)
	if err != nil {
	...
	if !strings.Contains(stdout, "size: "+strconv.FormatInt(info.Size(), 10)) {
		t.Errorf("asmdb backup reported a size that is not the file's %d\nstdout: %s", info.Size(), stdout)
	}
	...
	if out, stderr, code := run(t, bin["asmdb"], nil, "check", "--file", to); code != 0 {
		t.Errorf("asmdb check --file exited %d\nstdout: %s\nstderr: %s", code, out, stderr)
	}
	...
	// one asmd can be pointed at — VACUUM INTO leaves it in rollback-journal
	// mode and asmd requires WAL.
	restored := t.TempDir()
...
func TestBackupRefusals(t *testing.T) {
	...
	t.Run("a missing directory names it and creates nothing", func(t *testing.T) {
		...
		_, stderr, code := run(t, asmdb, nil, "backup", "--db", dir, "--to", filepath.Join(missing, "cms.db"))
		if code == 0 {
			t.Fatal("exited 0")
		}
	...
	t.Run("an existing file is refused and the flag is named", func(t *testing.T) {
		...
		if _, stderr, code := run(t, asmdb, nil, "backup", "--db", dir, "--to", to); code != 0 {
		...
		_, stderr, code := run(t, asmdb, nil, "backup", "--db", dir, "--to", to)
		if code == 0 {
		...
		if _, stderr, code := run(t, asmdb, nil, "backup", "--db", dir, "--to", to, "--overwrite"); code != 0 {
	...
	t.Run("a --db that is not ours is refused the usual way", func(t *testing.T) {
		...
		if !strings.Contains(stderr, "application_id") {
			t.Errorf("stderr = %q, want the application_id refusal every other asmdb command gives", stderr)
		}
	...
	t.Run("check wants exactly one of --db and --file", func(t *testing.T) {
		_, stderr, code := run(t, asmdb, nil, "check")
		if code == 0 || !strings.Contains(stderr, "--file") {
			t.Errorf("asmdb check with no target exited %d\nstderr: %s", code, stderr)
		}
		_, stderr, code = run(t, asmdb, nil, "check", "--db", dir, "--file", filepath.Join(backups, "twice.db"))
		if code == 0 {
			t.Errorf("asmdb check accepted both --db and --file\nstderr: %s", stderr)
		}
	})
}
...
func TestBackupAcrossAPendingMigration(t *testing.T) {
	...
	t.Run("the backup is taken and reports the schema in the file", func(t *testing.T) {
		...
		stdout, stderr, code := run(t, asmdb, nil, "backup", "--db", dir, "--to", to)
		if code != 0 {
	...
	t.Run("and the backup can be read back", func(t *testing.T) {
		...
		stdout, stderr, code := run(t, asmdb, nil, "check", "--file", to)
		if code != 0 {
	...
	t.Run("serving still refuses the same database", func(t *testing.T) {
		// The policy is per-operation, not global. asmd interprets rows, so
		// invariant 21 is untouched by any of this.
		_, stderr, code := run(t, bin["asmd"], nil, "serve", "--db", dir,
			"--addr", "127.0.0.1:0", "--timeout", "5s")
		if code == 0 {
			t.Fatal("asmd started against a database at the wrong version")
		}
