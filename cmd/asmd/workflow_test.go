...
// M4's end-to-end test: earl driven against a asmd on a temporary database
...
func TestEarlWorkflowCycle(t *testing.T) {
	...
	// "asmdb seed" reports the workflow the migration created, so that "what
	// does a fresh database contain" is still one command's output.
	if got, _, _ := run(t, bin["asmdb"], nil, "seed", "--db", dir); !strings.Contains(got, "workflow: ") {
		t.Errorf("asmdb seed does not report the default workflow:\n%s", got)
	}
