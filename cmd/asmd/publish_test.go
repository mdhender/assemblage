...
// M9's end-to-end test: earl driven against a asmd on a temporary database
// (AGENTS.md, "Testing").
//
// It publishes a story, reads what appeared in the output tree, changes the
// slug and watches the file at the old address go away, and asks "asmdb check"
// what the tree and the database disagree about.
//
// The workers are running in this asmd, so the publish and the expiry happen
...
func TestEarlPublish(t *testing.T) {
	...
	// asmd refuses to create either root (invariant 19).
	scratch := t.TempDir()
	output := t.TempDir()
	...
	t.Run("asmdb check reconciles the tree", func(t *testing.T) {
		stdout, stderr, code := run(t, bin["asmdb"], nil, "check", "--db", dir, "--output", output)
		if code != 0 {
		...
		stdout, _, code = run(t, bin["asmdb"], nil, "check", "--db", dir, "--output", output)
		if code == 0 {
...
func TestServeRefusesAnOutputDirectoryThatIsNotThere(t *testing.T) {
	...
	if code == 0 {
		t.Fatal("asmd started with an output directory that is not there")
	}
...
// the asmd this test started; there is no synchronous moment to assert at. The
