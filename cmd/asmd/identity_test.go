...
// The M2 acceptance criteria that are about the processes: what "asmdb
...
//
// They live in cmd/asmd because the harness that builds and runs the three
...
func bootstrapped(t *testing.T, asmdb, email, password string) string {
	t.Helper()
	dir := initDB(t, asmdb)

	if _, stderr, code := run(t, asmdb, nil, "seed", "--db", dir); code != 0 {
		t.Fatalf("asmdb seed exited %d\nstderr: %s", code, stderr)
	}
	stdout, stderr, code := runStdin(t, asmdb, password,
		"bootstrap", "admin", "--db", dir, "--email", email, "--name", "Admin", "--password-stdin")
	if code != 0 {
		t.Fatalf("asmdb bootstrap admin exited %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	}
	return dir
}
...
func TestBootstrapAdmin(t *testing.T) {
	...
	t.Run("a generated password is printed once", func(t *testing.T) {
		dir := initDB(t, asmdb)
		stdout, stderr, code := run(t, asmdb,
	...
	t.Run("a password on a flag is rejected", func(t *testing.T) {
		...
		for _, flag := range []string{"--password", "--pass", "--pw"} {
			...
			stdout, stderr, code := run(t, asmdb, nil, args...)
			if code == 0 {
				t.Errorf("asmdb bootstrap admin %s exited 0\nstdout: %s", flag, stdout)
			}
		...
		stdout, stderr, _ := run(t, asmdb, nil, "bootstrap", "admin", "--help")
		help := stdout + stderr
...
// TestSeedIsIdempotent: "asmdb seed" runs more than once in a developer's life
...
func TestSeedIsIdempotent(t *testing.T) {
	...
	bin := buildCommands(t, t.TempDir())
	dir := initDB(t, bin["asmdb"])

	first, stderr, code := run(t, bin["asmdb"], nil, "seed", "--db", dir)
	if code != 0 {
	...
	second, stderr, code := run(t, bin["asmdb"], nil, "seed", "--db", dir)
	if code != 0 {
	...
	if _, stderr, code := run(t, bin["asmdb"], nil, "check", "--db", dir); code != 0 {
		t.Fatalf("asmdb check after two seeds exited %d\nstderr: %s", code, stderr)
	}
}
...
func TestDevLoginRoute(t *testing.T) {
	...
	dir := bootstrapped(t, bin["asmdb"], email, "correct horse battery")
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			proc := start(t, bin["asmd"], tc.env, args...)
...
// end-to-end test: earl driven against a asmd on a temporary database.
//
...
func TestSeedSiteDomain(t *testing.T) {
	...
	t.Run("the domain given is the domain written", func(t *testing.T) {
		dir := initDB(t, bin["asmdb"])
		stdout, stderr, code := run(t, bin["asmdb"], nil, "seed", "--db", dir,
	...
	t.Run("a name that could not be a host is refused before anything is written", func(t *testing.T) {
		dir := initDB(t, bin["asmdb"])
		_, stderr, code := run(t, bin["asmdb"], nil, "seed", "--db", dir, "--site-domain", "not a host")
		if code == 0 {
	...
	t.Run("seeding again with another domain does not create a second site", func(t *testing.T) {
		dir := initDB(t, bin["asmdb"])
		if _, stderr, code := run(t, bin["asmdb"], nil, "seed", "--db", dir); code != 0 {
			t.Fatalf("seed exited %d\nstderr: %s", code, stderr)
		}
		_, stderr, code := run(t, bin["asmdb"], nil, "seed", "--db", dir, "--site-domain", "www.example.com")
		if code == 0 {
		...
		if _, stderr, code := run(t, bin["asmdb"], nil, "check", "--db", dir); code != 0 {
			t.Fatalf("asmdb check after the refusal exited %d\nstderr: %s", code, stderr)
		}
	})
}
