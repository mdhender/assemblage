...
const commands = "asmd asmdb earl"
...
// how a password reaches "asmdb bootstrap admin" and "earl login". A password
...
//
// It runs the real "asmdb init" rather than reaching into internal/store,
...
func TestCommands(t *testing.T) {
	...
	t.Run("dev shutdown flushes then exits 0", func(t *testing.T) {
		proc := start(t, bin["asmd"], nil, "serve", "--db", initDB(t, bin["asmdb"]), "--env", "development", "--addr", "127.0.0.1:0", "--timeout", "60s")
	...
	t.Run("timeout exits 0", func(t *testing.T) {
		began := time.Now()
		proc := start(t, bin["asmd"], nil, "serve", "--db", initDB(t, bin["asmdb"]), "--addr", "127.0.0.1:0", "--timeout", "2s")
...
// process is a asmd started by a test, with its listen address discovered from
...
func start(t *testing.T, bin string, env []string, args ...string) *process {
	...
	case addr, ok := <-addrs:
		if !ok {
			t.Fatalf("asmd exited before it logged a listen address\nstderr: %s", p.errb.String())
		}
		p.url = "http://" + addr
	case <-time.After(20 * time.Second):
		_ = cmd.Process.Kill()
		t.Fatalf("asmd did not log a listen address\nstderr: %s", p.errb.String())
	}
...
func (p *process) wait(t *testing.T, within time.Duration) int {
	...
	if !exited {
		...
		t.Fatalf("asmd did not exit within %s\nstderr: %s", within, p.errb.String())
	}
	if err == nil {
		return 0
	}
	...
	t.Fatalf("waiting for asmd: %v", err)
	return -1
}
...
// process a test starts, so that a failing assertion never leaves a asmd
// holding a port.
func (p *process) stop() {
	p.mu.Lock()
	done := p.waited
