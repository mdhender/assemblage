...
// M8's end-to-end test: earl driven against a asmd on a temporary database
...
// templateTree is the checked-in fixture asmd is pointed at.
//
...
func TestEarlPreview(t *testing.T) {
	...
	// needs no mkdir: asmd refuses to create one (invariant 19).
	scratch := t.TempDir()
...
func TestAsmdRefusesAMissingTemplateTree(t *testing.T) {
	...
	bin := buildCommands(t, t.TempDir())
	dir := bootstrapped(t, bin["asmdb"], "admin@example.com", "correct horse battery")

	missing := filepath.Join(t.TempDir(), "no-such-tree")
	for _, flag := range []string{"--templates", "--preview"} {
		stdout, stderr, code := run(t, bin["asmd"], nil,
			"serve", "--db", dir, "--addr", "127.0.0.1:0", "--timeout", "5s", flag, missing)
		if code == 0 {
			t.Errorf("asmd serve %s %s started; the directory does not exist\n%s", flag, missing, stdout)
		}
