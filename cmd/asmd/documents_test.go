...
// M3's end-to-end test: earl driven against a asmd on a temporary database
...
//
//	go test ./cmd/asmd/ -run TestEarlDocumentCycle -update
...
// TestSeedDemo covers "asmdb seed --demo", which is real now that there are
...
func TestSeedDemo(t *testing.T) {
	...
	t.Run("with no user it refuses and says why", func(t *testing.T) {
		dir := initDB(t, asmdb)
		if _, stderr, code := run(t, asmdb, nil, "seed", "--db", dir, "--demo"); code == 0 {
	...
	t.Run("with an administrator it seeds one document, idempotently", func(t *testing.T) {
		dir := bootstrapped(t, asmdb, "admin@example.com", "correct horse battery")

		stdout, stderr, code := run(t, asmdb, nil, "seed", "--db", dir, "--demo")
		if code != 0 {
