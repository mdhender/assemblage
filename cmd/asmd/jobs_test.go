...
// M6's end-to-end test: earl driven against a asmd on a temporary database
// (AGENTS.md, "Testing").
//
// The job it watches comes from "asmdb seed --demo", because nothing else can
...
// workers are hosted in asmd behind --workers, and the retry route refuses
// what it should.
func TestEarlJobs(t *testing.T) {
	...
	if stdout, stderr, code := run(t, bin["asmdb"], nil, "seed", "--demo", "--db", dir); code != 0 {
		t.Fatalf("asmdb seed --demo exited %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
	} else if !strings.Contains(stdout, "noop") {
		t.Fatalf("asmdb seed --demo queued no job:\n%s", stdout)
	}
	...
	// asmd processes behind one proxy wants the workers in one of them.
	idle := start(t, bin["asmd"], nil, "serve", "--db", dir,
	...
	if code := idle.wait(t, 30*time.Second); code != 0 {
		t.Fatalf("asmd exited %d after a shutdown request\nstderr: %s", code, idle.stderr())
	}

	working := start(t, bin["asmd"], nil, "serve", "--db", dir,
	...
	t.Run("the workers hosted in asmd run the job", func(t *testing.T) {
	...
	t.Run("asmdb check reports the queue", func(t *testing.T) {
		// It runs against a database another process has open, which is what
		// WAL and the busy timeout are for.
		stdout, stderr, code := run(t, bin["asmdb"], nil, "check", "--db", dir)
		if code != 0 {
			t.Fatalf("asmdb check exited %d\nstdout: %s\nstderr: %s", code, stdout, stderr)
		}
