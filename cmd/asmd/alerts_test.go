...
// M12's end-to-end test: earl driven against a asmd on a temporary database
...
// test that proves the dispatcher is actually wired into "asmd serve": every
...
func TestEarlAlertsAndNotifications(t *testing.T) {
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatal("no notification arrived within 30s; the dispatcher is not running inside asmd serve")
		}
