...
This document describes Assemblage, the content management system in
`github.com/mdhender/assemblage`, and why we are building it. It is the
reference for every implementation decision in this repository. `docs/PLAN.md`
sequences the work; `AGENTS.md` states the rules you must follow while doing it.
...
- The old `cli/` and single-binary command skeleton are superseded by the three
  commands in §11. Reuse the cobra patterns; do not preserve the structure.
...
alike. `internal/migrate/schema/0001_initial.sql` defines the final index.
...
Every database this system creates carries **application ID `0x41534D30`** —
the ASCII bytes of `ASM0` big-endian, `1095978288` decimal. It is a
