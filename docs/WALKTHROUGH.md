...
The screens are illustrative. What the story walks through is the engine: the
greyed action bar is `workflow.Available`, which `GET
/documents/{uid}/transitions` returns with a reason on every refused move.
