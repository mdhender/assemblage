...
func (s *Server) buildRoutes() (*http.ServeMux, []Route) {
	...
	// "asmd routes" builds a table without opening a database, and a route
...
// is what "asmd routes" does -- has the shutdown route and nothing else, and
// the table says so.
func (s *Server) devDeps() devroutes.Deps {
	...
	if s.svc == nil {
		// Route-table mode: the pattern is declared so that "asmd routes"
...
// process is up. It says nothing about the database, which asmd does not open
// until M1.
