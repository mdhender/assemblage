...
import (
	...
	"github.com/mdhender/assemblage/internal/service"
)
...
type Options struct {
	...
	// asmd passes os.Stderr. See Server.WriteBanner for why this is not just a
	// log line.
	Banner io.Writer
	...
	// without the handlers that would need it, which is what "asmd routes"
	...
	//
	// It exists for "asmd routes", whose whole job is to print what "serve"
	...
	// -- which is "--workers 0" and "asmd routes" alike.
	//
...
// Server is asmd's HTTP server: one route table, one shutdown path.
type Server struct {
...
// "asmd routes" can ask for it without binding a port.
...
// startup banner uses. "asmd routes" prints it above the table, so the table
...
func (s *Server) logStart(bound net.Addr) {
	...
	s.log.Info("asmd starting",
		"environment", s.env.String(),
		"addr", bound.String(),
		"routes", len(s.routes),
	)
	if !config.AddrIsLoopback(bound.String()) {
		s.log.Warn("listening on a non-loopback address; asmd speaks plain HTTP and never terminates TLS",
			"addr", bound.String(),
		)
	}
}
