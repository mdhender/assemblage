...
// applied through the same code path asmdb uses and with foreign keys on
...
// 0001_initial.sql. Everything above it assumes there is exactly one draft to
...
// TestElementTypes covers the read paths the API and "asmdb seed" use.
func TestElementTypes(t *testing.T) {
	...
	// A duplicate key name is a conflict, which is what makes "asmdb seed"
