...
func TestCreate(t *testing.T) {
	...
	// refusal asmdb turns into "already initialised".
...
func TestOpenMissingDatabase(t *testing.T) {
	...
	for _, want := range []string{Path(dir), "asmdb init", dir} {
...
func TestOpenVersionMismatch(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if verErr.Behind() && !strings.Contains(msg, "asmdb migrate up") {
...
// TestOpenAllowBehind is the other half of the policy: "asmdb migrate status"
// and "asmdb migrate up" must be able to open the database they are there to
...
// TestMigrateUp is what "asmdb migrate up" does, including --to.
...
// TestCheck is what "asmdb check" reports on a healthy database.
...
// file uses: all migrations applied through the same runner asmdb uses, and
