...
const (
	// RequireExact rejects anything but an exact match. This is asmd
	// (invariant 21) and every asmdb subcommand that reads the schema, because
	...
	// "asmdb migrate status" can report them and "asmdb migrate up" can apply
	...
	// interpret its rows: "asmdb backup", which is a VACUUM INTO, and
	// "asmdb check --file", which asks SQLite whether a file is sound. Neither
	...
	// system that accepts it. Copying a newer file with an older asmdb
...
// Options are the knobs on Open. The zero value is what asmd wants: an exact
...
// that creates what it cannot find turns "asmd serve --db ./vsr" into a
...
// file, and "asmdb init" is its only caller (invariant 20). It fails if dir
// does not exist and it never creates one. It fails if the database already
// exists, because creating something twice is not creation; "asmdb init"
...
func Create(ctx context.Context, dir string) (*DB, error) {
	...
	// through the same verification as what asmd opens. A create that produced
...
// it can adopt a freshly created empty file. That is right for "asmdb init"
...
//
// Only asmdb reaches this. asmd opens and verifies and does not own a way to
