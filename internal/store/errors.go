...
func (e *NotFoundError) Error() string {
	return fmt.Sprintf("database %q does not exist: run \"asmdb init --db %s\"", e.Path, e.Dir)
}

func (e *NotFoundError) Unwrap() error { return e.Err }

// ExistsError reports that "asmdb init" was pointed at a directory that
...
func (e *AppIDError) Error() string {
	return fmt.Sprintf("database %q: application_id is %s, expected %s; this file was not created by asmdb init",
...
// the database. Neither is asmd's to repair: a server that migrates on startup
...
// case an operator fixes with "asmdb migrate up".
...
func (e *VersionError) Error() string {
	...
	if e.Behind() {
		remedy = fmt.Sprintf("run \"asmdb migrate up --db %s\"", e.Dir)
	}
