...
// so the timeout is here for the other process holding the file: a asmdb
// running beside a asmd.
...
type DB struct {
	...
	// a DB is open: asmd never migrates, so the value a server reads is the
