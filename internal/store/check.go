...
// CheckReport is what "asmdb check" found. It is a value rather than printed
...
type CheckReport struct {
	...
	// never heard of and has no business opening. "asmdb check --output DIR"
...
// (invariant 3): "asmdb check" holds the real clock and hands it down, and a
...
func CheckFile(ctx context.Context, path string, now time.Time) (*CheckReport, error) {
	...
	// SQLITE_CANTOPEN or, worse, NotFoundError's advice to run "asmdb init"
	...
	// other asmdb command, because it is the same check (DESIGN.md 13.4).
