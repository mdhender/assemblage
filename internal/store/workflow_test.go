...
// applied through the same runner asmdb uses and with foreign keys on
...
// TestDocumentsAndVersionsRemainConsistent verifies the initial schema's
// document/version cycle and composite workflow-state foreign key together.
func TestDocumentsAndVersionsRemainConsistent(t *testing.T) {
	dir := t.TempDir()

	// A fresh database with the complete initial schema.
	...
	doc, draft := f.create(t, "Consistent References")
	...
	// The document carries its workflow columns and its versions remain
	// reachable through the circular document/version references.
	...
	if !report.OK() {
		t.Errorf("the database does not check out: %+v", report)
	}
}

// TestCompositeForeignKeyRefusesAnUndeclaredState verifies that the pair
// (workflow_id, state) is checked by the database, so a state no workflow
// declares cannot be stored whatever the Go code believes.
