...
// path asmdb uses and with foreign keys on (DESIGN.md 15, invariant 22).
...
// TestUsers covers the round trip and the constraint that makes "asmdb
...
func TestIdentityCarriesRolesAndGrants(t *testing.T) {
	...
	// for, and "asmdb seed" runs more than once.
	if err := db.AssignRole(t.Context(), u.ID, editor.ID); err != nil {
