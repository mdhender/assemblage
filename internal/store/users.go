...
// what makes "asmdb bootstrap admin" idempotent without a read-then-write race.
...
// one "asmdb bootstrap admin" prints once at creation. Creating users nobody
...
// CountUsers reports how many users exist, which is what "asmdb bootstrap
...
// FirstUser reads the oldest user, which is the administrator "asmdb bootstrap
// admin" created. It exists so that "asmdb seed --demo" has somebody to
