...
func (db *DB) Publish(ctx context.Context, req PublishRequest) (PublishResult, error) {
	...
	err := db.Tx(ctx, func(conn *sqlite.Conn) error {
		...
		// briefly did not exist is a window in which "asmdb check" would call
...
//
// It is what "asmdb check" reconciles the output tree against
...
// an operator deleted by hand -- is what "asmdb check --output" reports rather
