...
// These are #10: asmdb had no backup command, and the deploy doc's sqlite3 is
...
func TestBackupWritesAVerifiedSnapshot(t *testing.T) {
	...
	fillWAL(t, db, rows)
	readOnce(t, db)
	if n := sizeOf(t, Path(dir)+"-wal"); n <= 0 {
		t.Fatalf("%s-wal is %d bytes; the snapshot would not be proving anything", FileName, n)
	}
...
func TestBackupCreatesNoDirectory(t *testing.T) {
	...
	_, err := db.Backup(t.Context(), Path(missing), BackupOptions{Now: time.Now().UTC()})
	var dirErr *DirError
...
// prove is that the file this writes is one asmd can be pointed at: VACUUM
...
// asked for, rather than NotFoundError's advice to run "asmdb init" against a
...
func TestCheckFileNamesAMissingFile(t *testing.T) {
	...
	if errors.As(err, &notFound) {
		t.Error("CheckFile returned a *NotFoundError, whose remedy is \"asmdb init\" against a --db directory")
	}
}
...
// construction. Requiring them to agree made "asmdb backup" refuse exactly
...
func TestBackupTakesAnyVersion(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		// an older asmdb produces a correct copy of a newer file; refusing it
		// would be the same mistake pointing the other way.
		{"ahead, which is an older asmdb against a migrated database", int32(migrate.Count()) + 1},
	} {
...
// they ask about.
//
// A legacy database can predate the jobs table. Counting stuck leases there is
