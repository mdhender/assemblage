...
func (s *Service) Preview(ctx context.Context, actor domain.Identity, req PreviewRequest) (PreviewResult, error) {
	if s.renderer == nil {
		return PreviewResult{}, fmt.Errorf(
			"this server was started without a template tree, so it renders nothing; give asmd --templates DIR: %w",
	...
	if s.preview == nil {
		return PreviewResult{}, fmt.Errorf(
			"this server was started without a preview tree, so a preview has nowhere to go; give asmd --preview DIR: %w",
			domain.ErrUnavailable)
	}
...
func (s *Service) PreviewOpen(name string) (fs.File, error) {
	if s.preview == nil {
		return nil, fmt.Errorf(
			"this server was started without a preview tree; give asmd --preview DIR: %w",
			domain.ErrUnavailable)
	}
	return s.preview.Open(name)
}

// PreviewConfigured reports whether this server can serve previews at all,
