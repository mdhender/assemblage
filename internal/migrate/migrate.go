...
// ASCII bytes of "ASM0" big-endian, 0x41534D30, 1095978288 decimal
// (DESIGN.md 13.2).
//
...
// thing standing between asmd and a file belonging to some other program.
const AppID int32 = 0x41534D30
...
// key enforcement to be off while it runs. The initial schema does not need
// it, but a future table rebuild may: SQLite's documented twelve-step ALTER
// procedure begins by turning foreign keys off.
//
// It is a directive in the file rather than a table in this package because a
// list kept somewhere else is a list that drifts from the files it describes.
...
type Migration struct {
	...
	// "-- migrate: disable-foreign-keys" directive.
...
// PRAGMA user_version a fully migrated database carries. asmd requires the two
...
// them.
//
// This is the only place migrations are applied. asmd never calls it —
// asmd opens and verifies, and repairing a version mismatch is the operator's
...
// Describe renders the migration list for "asmdb migrate status".
