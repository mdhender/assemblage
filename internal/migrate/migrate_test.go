...
// ways, and all three have to agree: 0x41534D30, 1095978288, and the ASCII
// bytes of "ASM0".
func TestAppID(t *testing.T) {
	if AppID != 0x41534D30 {
		t.Errorf("AppID = %#x, want 0x41534D30", AppID)
	}
	if AppID != 1095978288 {
		t.Errorf("AppID = %d, want 1095978288", AppID)
	}
	want := "ASM0"
	id := uint32(AppID)
	...
	if s := AppIDString(AppID); !strings.Contains(s, "ASM0") || !strings.Contains(s, "0x41534d30") {
...
func TestEmbeddedMigrations(t *testing.T) {
	all := All()
	if len(all) != 1 || all[0].Name != "0001_initial.sql" {
		t.Fatalf("embedded migrations = %v, want only 0001_initial.sql", Names())
	}
	if Count() != len(all) {
...
// than for today's migration count.
...
// TestPending is what "asmdb migrate status" reports.
...
// understands. The squashed initial schema creates final tables directly, so
// it must not disable referential-integrity checks.
func TestDisableForeignKeysDirective(t *testing.T) {
	...
	if len(carrying) != 0 {
		t.Fatalf("migrations carrying the disable-foreign-keys directive: %v, want none", carrying)
	}
