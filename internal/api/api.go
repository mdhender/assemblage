...
// "asmd routes" prints is produced by the same call that registers the
...
func Register(mux Mux, deps Deps) {
	...
	//
	// That is for "asmd routes", whose job is to print the table "serve" would
...
//
// Nothing serves such a table today -- "asmd serve" opens the database before
